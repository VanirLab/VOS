...
#define VANIRSD_SOCKET "/var/run/vanirsd.sock"
...
int main(int argc, char **argv) {
    ...
    // now, read from stdin and write it to vanirsd
	...
	// end of request, now let vanirsd execute the action and return response
    shutdown(fd, SHUT_WR);

    // then, retrieve the response from vanirsd and send it to stdout
    while ((read_ret = read(fd, buf, sizeof(buf))) > 0)
        write_wrapper(1, buf, read_ret);

    if (read_ret == -1) {
        perror("read from vanirsd");
        return 1;
    }
