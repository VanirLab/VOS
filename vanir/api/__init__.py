...
class AbstractVanirAPI:
    '''Common code for Vanir Management Protocol handling
...
class VanirDaemonProtocol(asyncio.Protocol):
    buffer_size = 65536
    ...
    def respond(self, src, meth, dest, arg, *, untrusted_payload):
        ...
        except vanir.exc.VanirException as err:
            msg = ('%r while calling '
...
def create_servers(*args, force=False, loop=None, **kwargs):
    '''Create multiple Vanir API servers
    :param vanir.Vanir app: the app that is a backend of the servers
    ...
    *args* are supposed to be classes inheriting from
    :py:class:`AbstractVanirAPI`
    *kwargs* (like *app* or *debug* for example) are passed to
    :py:class:`VanirDaemonProtocol` constructor
    '''
    ...
    try:
        ...
        for handler in args:
            ...
            server = yield from loop.create_unix_server(
                functools.partial(VanirDaemonProtocol, handler, **kwargs),
                sockpath)

            for sock in server.sockets:
                shutil.chown(sock.getsockname(), group='vanir')
