...
Such extension should provide:
 - `vanir.devices` endpoint - a class descendant from
 :py:class:`vanir.devices.DeviceInfo`, designed to hold device description (
...
this into account (for example by using VanirDB).
Extension may use VanirDB watch API (vanirVM.watch_qdb_path(path), then handle
...
class DeviceNotAttached(vanir.exc.VanirException, KeyError):
    '''Trying to detach not attached device'''

class DeviceAlreadyAttached(vanir.exc.VanirException, KeyError):
...
class DeviceCollection:
    ...
    def __init__(self, vm, bus):
        ...
        self.devclass = vanir.utils.get_entry_point_one(
            'vanir.devices', self._bus)
    ...
    def load_persistent(self, device_assignment: DeviceAssignment):
        '''Load DeviceAssignment retrieved from vanir.xml
        This can be used only for loading vanir.xml, when VM events are not
        enabled yet.
        '''
    ...
    def assignments(self, persistent=None):
        ...
        :file:`vanir.xml`) or not. Device can also be in :file:`vanir.xml`,
...
class PersistentCollection:
    ...
    def get(self, device: DeviceInfo) -> DeviceAssignment:
        ''' Returns the corresponding `vanir.devices.DeviceAssignment` for the
            device. '''
