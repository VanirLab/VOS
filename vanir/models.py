...
class PackageManager(models.Manager):
    ...
    def normal(self):
        return self.select_related('vanir', 'repo')
...
class Package(models.Model):
    ...
    vanir = models.ForeignKey(Vanir, related_name="packages",
            on_delete=models.PROTECT)
    ...
    class Meta:
        ...
        unique_together = (('pkgname', 'repo', 'vanir'),)
    ...
    def applicable_arches(self):
        '''The list of (this vanir) + (available agnostic arches).'''
        if self._applicable_arches is None:
            arches = set(Vanir.objects.filter(agnostic=True))
            arches.add(self.vanir)
    ...
    def get_requiredby(self):
        ...
        if not Package.objects.filter(
                repo_q, pkgname=self.pkgname, vanir=self.vanir
    ...
    def base_package(self):
        ...
        try:
            ...
            return Package.objects.normal().get(vanir=self.vanir,
            ...
            pkglist = Package.objects.normal().filter(vanir=self.vanir,
    ...
    def in_testing(self):
        ...
        try:
            return Package.objects.normal().get(repo__testing=True,
                    pkgname=self.pkgname, vanir=self.vanir)
        except Package.DoesNotExist:
            return None
