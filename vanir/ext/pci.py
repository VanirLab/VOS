...
def pcidev_class(dev_xmldesc):
    ...
    if not vanir.ext.pci.pci_classes:
        vanir.ext.pci.pci_classes = load_pci_classes()
    if class_id.startswith('0x'):
        class_id = class_id[2:]
    try:
        # ignore prog-if
        return vanir.ext.pci.pci_classes[class_id[0:4]]
    except KeyError:
        return "Unknown"
    ...
    for domid in xs.ls('', 'backend/pci') or []:
        for devid in xs.ls('', 'backend/pci/' + domid) or []:
            ...
            try:
                ...
                # unknown domain - maybe from another vanir.xml?
                continue
...
class PCIDevice(vanir.devices.DeviceInfo):
...
class PCIDeviceExtension(vanir.ext.Extension):
    def __init__(self):
        super(PCIDeviceExtension, self).__init__()
        # lazy load this
        self.pci_classes = {}

    @vanir.ext.handler('device-list:pci')
    ...
    @vanir.ext.handler('device-get:pci')
    def on_device_get_pci(self, vm, event, ident):
        # pylint: disable=unused-argument,no-self-use
        if not vm.app.vmm.offline_mode:
            yield _cache_get(vm, ident)

    @vanir.ext.handler('device-list-attached:pci')
    def on_device_list_attached(self, vm, event, **kwargs):
        # pylint: disable=unused-argument,no-self-use
        if not vm.is_running() or isinstance(vm, vanir.vm.adminvm.AdminVM):
            return
    ...
    @vanir.ext.handler('device-pre-attach:pci')
    ...
    @vanir.ext.handler('device-pre-detach:pci')
    def on_device_pre_detached_pci(self, vm, event, device):
        ...
        # vanir.DetachPciDevice, which unbinds driver, not to oops the kernel
        ...
        vmdev = m.group(1)
        try:
            vm.run_service('vanir.DetachPciDevice',
    ...
    @vanir.ext.handler('domain-pre-start')
    ...
    def bind_pci_to_pciback(app, device):
        ...
        :param vanir.devices.PCIDevice device: device to attach
