...
import vanir.backup
import vanir.exc
import vanir.storage.lvm
import vanir.tests
import vanir.tests.storage_lvm
import vanir.vm
...
try:
    import vaniradmin.backup.restore
    import vaniradmin.exc
...
class BackupTestsMixin(object):
    ...
    def setUp(self):
        ...
        backup_log = logging.getLogger('vaniradmin.backup')
        backup_log.addHandler(self.error_handler)

    def tearDown(self):
        shutil.rmtree(self.backupdir)

        backup_log = logging.getLogger('vanir.backup')
    ...
    def create_backup_vms(self, pool=None):
        ...
        testnet = self.app.add_new_vm(vanir.vm.appvm.AppVM,
        ...
        self.log.debug("Creating %s" % vmname)
        testvm1 = self.app.add_new_vm(vanir.vm.appvm.AppVM,
        ...
        self.log.debug("Creating %s" % vmname)
        testvm2 = self.app.add_new_vm(vanir.vm.standalonevm.StandaloneVM,
                                      name=vmname,
                                      virt_mode='hvm',
                                      label='red')
        self.loop.run_until_complete(
        ...
        self.log.debug("Creating %s" % vmname)
        testvm3 = self.app.add_new_vm(vanir.vm.templatevm.TemplateVM,
        ...
        self.log.debug("Creating %s" % vmname)
        testvm4 = self.app.add_new_vm(vanir.vm.appvm.AppVM,
    ...
    def make_backup(self, vms, target=None, expect_failure=False, **kwargs):
        ...
        try:
            backup = vanir.backup.Backup(self.app, vms, **kwargs)
        except vanir.exc.VanirException as e:
            if not expect_failure:
                self.fail("VanirException during backup_prepare: %s" % str(e))
            else:
                raise

        if 'passphrase' not in kwargs:
            backup.passphrase = 'vanir'
        ...
        try:
            ...
            if not expect_failure:
                self.fail("VanirException during backup_do: %s" % str(e))
            else:
                raise
    ...
    def restore_backup(self, source=None, appvm=None, options=None,
        ...
        if not restore_available:
            self.skipTest('vaniradmin module not available')
    ...
    def vm_checksum(self, vms):
        hashes = {}
        for vm in vms:
            assert isinstance(vm, vanir.vm.vanirvm.VanirVM)
            hashes[vm.name] = {}
    ...
    def assertCorrectlyRestored(self, vms_info, orig_hashes):
        ...
        :param orig_vms: collection of original VanirVM objects
...
class TC_00_Backup(BackupTestsMixin, vanir.tests.SystemTestCase):
    ...
    def test_004_sparse_multipart(self):
        ...
        try:
            ...
            self.hvmtemplate = self.app.add_new_vm(
                vanir.vm.templatevm.TemplateVM, name=vmname, virt_mode='hvm', label='red')
        ...
        try:
            ...
            for vm_name in vms_info:
                with self.assertNotRaises(
                        (vanir.exc.VanirVMNotFoundError, KeyError)):
    ...
    @vanir.tests.storage_lvm.skipUnlessLvmPoolExists
    def test_300_backup_lvm(self):
        volume_group, thin_pool = \
            vanir.tests.storage_lvm.DEFAULT_LVM_POOL.split('/', 1)
        self.pool = self._find_pool(volume_group, thin_pool)
        if not self.pool:
            self.pool = self.loop.run_until_complete(
                self.app.add_pool(
                    **vanir.tests.storage_lvm.POOL_CONF))
    ...
    def test_100_send_to_vm_file_with_spaces(self):
        ...
        try:
            ...
            (backup_path, _) = self.loop.run_until_complete(
                self.backupvm.run_for_stdio("ls /var/tmp/backup*/vanir-backup*"))
...
def create_testcases_for_templates():
    return vanir.tests.create_testcases_for_templates('TC_10_BackupVM',
        TC_10_BackupVMMixin, vanir.tests.SystemTestCase,
