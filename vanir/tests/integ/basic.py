...
class TC_00_Basic(vanir.tests.SystemTestCase):
    def setUp(self):
    ...
    def test_100_qvm_create(self):
        ...
        vm = self.app.add_new_vm(vanir.vm.appvm.AppVM,
...
class TC_01_Properties(vanir.tests.SystemTestCase):
    # pylint: disable=attribute-defined-outside-init
    def setUp(self):
        super(TC_01_Properties, self).setUp()
        self.init_default_template()
        self.vmname = self.make_vm_name('appvm')
        self.vm = self.app.add_new_vm(vanir.vm.appvm.AppVM, name=self.vmname,
                                      template=self.app.default_template,
                                      label='red')
        self.loop.run_until_complete(self.vm.create_on_disk())
    ...
    def test_030_clone(self):
        try:
            ...
            firewall.rules = [
                vanir.firewall.Rule(None, action='accept', dsthost='1.2.3.0/24',
    ...
    def test_020_name_conflict_app(self):
        ...
        with self.assertRaises((vanir.exc.VanirException, ValueError)):
            self.vm2 = self.app.add_new_vm(vanir.vm.appvm.AppVM,
    ...
    def test_021_name_conflict_template(self):
        # TODO decide what exception should be here
        with self.assertRaises((vanir.exc.VanirException, ValueError)):
            self.vm2 = self.app.add_new_vm(vanir.vm.templatevm.TemplateVM,
                name=self.vmname, label='red')
            self.loop.run_until_complete(self.vm2.create_on_disk())


class TC_03_QvmRevertTemplateChanges(vanir.tests.SystemTestCase):
    # pylint: disable=attribute-defined-outside-init
    ...
    def setup_template(self):
        self.test_template = self.app.add_new_vm(
            vanir.vm.templatevm.TemplateVM,
...
class TC_30_Gui_daemon(vanir.tests.SystemTestCase):
    def setUp(self):
    ...
    def test_000_clipboard(self):
        ...
        self.loop.run_until_complete(testvm1.create_on_disk())
        testvm2 = self.app.add_new_vm(vanir.vm.appvm.AppVM,
        ...
        clipboard_content = \
            open('/var/run/vanir/vanir-clipboard.bin', 'r').read().strip()
        self.assertEqual(clipboard_content, test_string,
                          "Clipboard copy operation failed - content")
        clipboard_source = \
            open('/var/run/vanir/vanir-clipboard.bin.source',
                 'r').read().strip()
        ...
        clipboard_source = \
            open('/var/run/vanir/vanir-clipboard.bin.source', 'r').\
            read().strip()
...
class TC_05_StandaloneVMMixin(object):
    ...
    def test_000_create_start(self):
        self.testvm1 = self.app.add_new_vm(vanir.vm.standalonevm.StandaloneVM,
                                     name=self.make_vm_name('vm1'), label='red')
    ...
    def test_100_resize_root_img(self):
        ...
        try:
            self.loop.run_until_complete(
                ...
                vanir.storage.StoragePoolException) as e:
...
def create_testcases_for_templates():
    yield from vanir.tests.create_testcases_for_templates('TC_05_StandaloneVM',
            TC_05_StandaloneVMMixin, vanir.tests.SystemTestCase,
            module=sys.modules[__name__])
    yield from vanir.tests.create_testcases_for_templates('TC_06_AppVM',
            TC_06_AppVMMixin, vanir.tests.SystemTestCase,
            module=sys.modules[__name__])
