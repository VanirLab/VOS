...
class StorageTestMixin(object):
    def setUp(self):
        ...
        self.vm1 = self.app.add_new_vm(vanir.vm.appvm.AppVM,
    ...
    def tearDown(self):
        with suppress(vanir.exc.VanirException):
...
class StorageFile(StorageTestMixin, vanir.tests.SystemTestCase):
    def init_pool(self):
...
class StorageReflinkMixin(StorageTestMixin):
    ...
    def init_pool(self, fs_type, **kwargs):
        ...
        vanir.tests.storage_reflink.mkdir_fs(dir_path, fs_type,
                                             cleanup_via=self.addCleanup)
        self.pool = self.loop.run_until_complete(
            self.app.add_pool(name=name, dir_path=dir_path,
                              driver='file-reflink', **kwargs))

class StorageReflinkOnBtrfs(StorageReflinkMixin, vanir.tests.SystemTestCase):
    def init_pool(self):
        super().init_pool('btrfs')

class StorageReflinkOnExt4(StorageReflinkMixin, vanir.tests.SystemTestCase):
    def init_pool(self):
        super().init_pool('ext4', setup_check='no')


@vanir.tests.storage_lvm.skipUnlessLvmPoolExists
class StorageLVM(StorageTestMixin, vanir.tests.SystemTestCase):
    def init_pool(self):
        self.created_pool = False
        # check if the default LVM Thin pool vanir_dom0/pool00 exists
        volume_group, thin_pool = \
            vanir.tests.storage_lvm.DEFAULT_LVM_POOL.split('/', 1)
        self.pool = self._find_pool(volume_group, thin_pool)
        if not self.pool:
            self.pool = self.loop.run_until_complete(
                self.app.add_pool(**vanir.tests.storage_lvm.POOL_CONF))
