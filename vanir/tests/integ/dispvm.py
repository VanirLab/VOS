...
class TC_04_DispVM(vanir.tests.SystemTestCase):

    def setUp(self):
        super(TC_04_DispVM, self).setUp()
        self.init_default_template()
        self.disp_base = self.app.add_new_vm(vanir.vm.appvm.AppVM,
        ...
        self.testvm = self.app.add_new_vm(vanir.vm.appvm.AppVM,
        ...
        try:
            (stdout, _) = self.loop.run_until_complete(
                self.testvm.run_for_stdio("qvm-run-vm --dispvm bash",
                    input=b"echo test; vanirdb-read /name; echo ERROR\n"))
    ...
    def test_003_cleanup_destroyed(self):
        ...
        p.stdin.write(b"vanirdb-read /name\n")
...
class TC_20_DispVMMixin(object):
    ...
    def test_010_simple_dvm_run(self):
        dispvm = self.loop.run_until_complete(
            vanir.vm.dispvm.DispVM.from_appvm(self.disp_base))
        try:
            self.loop.run_until_complete(dispvm.start())
            (stdout, _) = self.loop.run_until_complete(
                dispvm.run_service_for_stdio('vanir.VMShell',
                    input=b"echo test"))
    ...
    def test_020_gui_app(self):
        ...
        try:
            ...
            p = self.loop.run_until_complete(
                dispvm.run_service('vanir.VMShell',
        ...
        self.assertNotIn(dispvm_name, self.app.domains,
                          "DispVM not removed from vanir.xml")
...
def create_testcases_for_templates():
    return vanir.tests.create_testcases_for_templates('TC_20_DispVM',
        TC_20_DispVMMixin, vanir.tests.SystemTestCase,
