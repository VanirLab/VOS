...
class TC_00_FilePool(vanir.tests.VanirTestCase):
    ...
    def test000_default_pool_dir(self):
        """ The predefined dir for the default pool should be ``/var/lib/vanir``

            .. sealso::
               Data :data:``vanir.vanir.defaults['pool_config']``.
        """
        result = self.app.get_pool("varlibqubes").dir_path
        expected = '/var/lib/vanir'
        self.assertEqual(result, expected)

    def test001_default_storage_class(self):
        """ Check when using default pool the Storage is
            ``vanir.storage.Storage``. """
        result = self._init_app_vm().storage
        self.assertIsInstance(result, vanir.storage.Storage)

    def _init_app_vm(self):
        """ Return initalised, but not created, AppVm. """
        vmname = self.make_vm_name('appvm')
        self.app.create_dummy_template()
        return self.app.add_new_vm(vanir.vm.appvm.AppVM, name=vmname,
                                   template=self.app.default_template,
                                   label='red')


class TC_01_FileVolumes(vanir.tests.VanirTestCase):
    ...
    def tearDown(self):
        ...
        for vm in list(self.app.domains):
            if vm.name.startswith(vanir.tests.VMPREFIX):
    ...
    def test_001_snapshot_volume(self):
        ...
        original_size = vanir.config.defaults['root_img_size']
        source_config = {
            'name': 'root',
            'pool': self.POOL_NAME,
            'save_on_stop': True,
        ...
        block = volume.block_device()
        assert isinstance(block, vanir.storage.BlockDevice)
        self.assertEqual(block.path,
    ...
    def test_003_read_only_volume(self):
        ...
        vm = vanir.tests.storage.TestVM(self, template=template)
    ...
    def test_005_appvm_volumes(self):
        ...
        vmname = self.make_vm_name('appvm')
        vm = self.app.add_new_vm(vanir.vm.appvm.AppVM, name=vmname,
                                 template=self.app.default_template,
                                 label='red')
    ...
    def test_006_template_volumes(self):
        ...
        vmname = self.make_vm_name('appvm')
        vm = self.app.add_new_vm(vanir.vm.templatevm.TemplateVM, name=vmname,
                                 label='red')
...
class TC_03_FilePool(vanir.tests.VanirTestCase):
    ...
    def tearDown(self):
        ...
        shutil.rmtree(self.POOL_DIR, ignore_errors=True)
        if os.path.exists('/tmp/vanir-test-dir'):
            shutil.rmtree('/tmp/vanir-test-dir')
