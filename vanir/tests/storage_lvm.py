''' Tests for lvm storage driver. By default tests are going to use the
    'vanir_dom0/pool00'. An alternative LVM thin pool may be provided via
    ...
    represent a :py:class:`vanir.storage.lvm.ThinPool`.
'''
import os
import subprocess
import tempfile
...
class TC_00_ThinPool(ThinPoolBase):
    ...
    def setUp(self):
        xml_path = '/tmp/vanir-test.xml'
        self.app = vanir.Vanir.create_empty_store(store=xml_path,
            clockvm=None,
            updatevm=None,
            offline_mode=True,
        )
    ...
    def tearDown(self):
        ...
        for attr in dir(self):
            if isinstance(getattr(self, attr), vanir.vm.BaseVM):
                delattr(self, attr)
    ...
    def test_001_origin_volume(self):
        ...
        self.assertEqual(volume.size, vanir.config.defaults['root_img_size'])
    ...
    def _get_lv_origin_uuid(self, lv):
        sudo = [] if os.getuid() == 0 else ['sudo']
        if vanir.storage.lvm.lvm_is_very_old:
...
@skipUnlessLvmPoolExists
class TC_01_ThinPool(ThinPoolBase, vanir.tests.SystemTestCase):
    ''' Sanity tests for :py:class:`vanir.storage.lvm.ThinPool` '''
    ...
    def test_004_import(self):
        ...
        vm = self.app.add_new_vm(vanir.vm.templatevm.TemplateVM, name=name,
                            label='red')
    ...
    def test_005_create_appvm(self):
        vm = self.app.add_new_vm(cls=vanir.vm.appvm.AppVM,
    ...
    def test_006_name_suffix_parse(self):
        ...
        vm1 = self.app.add_new_vm(cls=vanir.vm.appvm.AppVM,
            name=self.make_vm_name('appvm'), label='red')
        vm2 = self.app.add_new_vm(cls=vanir.vm.appvm.AppVM,
...
class TC_02_StorageHelpers(ThinPoolBase):
    ...
    def test_000_search_thin_pool(self):
        pool = vanir.storage.search_pool_containing_dir(
