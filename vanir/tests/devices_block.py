...
<domain type='xen' id='9'>
  ...
  <os>
    ...
    <kernel>/var/lib/vanir/vm-kernels/4.4.55-11/vmlinuz</kernel>
    <initrd>/var/lib/vanir/vm-kernels/4.4.55-11/initramfs</initrd>
  ...
  <devices>
    <disk type='block' device='disk'>
      <driver name='phy'/>
      <source dev='/var/lib/vanir/vm-templates/fedora-25/root.img:/var/lib/vanir/vm-templates/fedora-25/root-cow.img'/>
      <backingStore/>
    ...
    <disk type='block' device='disk'>
      <driver name='phy'/>
      <source dev='/var/lib/vanir/appvms/test-vm/private.img'/>
      <backingStore/>
      <target dev='xvdb' bus='xen'/>
    </disk>
    <disk type='block' device='disk'>
      <driver name='phy'/>
      <source dev='/var/lib/vanir/appvms/test-vm/volatile.img'/>
      <backingStore/>
      <target dev='xvdc' bus='xen'/>
    </disk>
    <disk type='block' device='disk'>
      <driver name='phy'/>
      <source dev='/var/lib/vanir/vm-kernels/4.4.55-11/modules.img'/>
      <backingStore/>
...
class TC_00_Block(vanir.tests.QubesTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.ext = vanir.ext.block.BlockDeviceExtension()

    def test_000_device_get(self):
        vm = TestVM({
            '/vanir-block-devices/sda': b'',
            '/vanir-block-devices/sda/desc': b'Test device',
            '/vanir-block-devices/sda/size': b'1024000',
            '/vanir-block-devices/sda/mode': b'w',
        })
        device_info = self.ext.device_get(vm, 'sda')
        self.assertIsInstance(device_info, vanir.ext.block.BlockDevice)
    ...
    def test_001_device_get_other_node(self):
        vm = TestVM({
            '/vanir-block-devices/mapper_dmroot': b'',
            '/vanir-block-devices/mapper_dmroot/desc': b'Test device',
            '/vanir-block-devices/mapper_dmroot/size': b'1024000',
            '/vanir-block-devices/mapper_dmroot/mode': b'w',
        })
    ...
    def test_002_device_get_invalid_desc(self):
        vm = TestVM({
            '/vanir-block-devices/sda': b'',
            '/vanir-block-devices/sda/desc': b'Test device<>za\xc4\x87abc',
            '/vanir-block-devices/sda/size': b'1024000',
            '/vanir-block-devices/sda/mode': b'w',
        })
        device_info = self.ext.device_get(vm, 'sda')
    ...
    def test_003_device_get_invalid_size(self):
        vm = TestVM({
            '/vanir-block-devices/sda': b'',
            '/vanir-block-devices/sda/desc': b'Test device',
            '/vanir-block-devices/sda/size': b'1024000abc',
            '/vanir-block-devices/sda/mode': b'w',
        })
        device_info = self.ext.device_get(vm, 'sda')
    ...
    def test_004_device_get_invalid_mode(self):
        vm = TestVM({
            ...
            '/vanir-block-devices/sda/desc': b'Test device',
            '/vanir-block-devices/sda/size': b'1024000',
            '/vanir-block-devices/sda/mode': b'abc',
        })
        device_info = self.ext.device_get(vm, 'sda')
        self.assertEqual(device_info.mode, 'w')
    ...
    def test_010_devices_list(self):
        vm = TestVM({
            ...
            '/vanir-block-devices/sda/size': b'1024000',
            '/vanir-block-devices/sda/mode': b'w',
            '/vanir-block-devices/sdb': b'',
            '/vanir-block-devices/sdb/desc': b'Test device2',
            '/vanir-block-devices/sdb/size': b'2048000',
            '/vanir-block-devices/sdb/mode': b'r',
        })
    ...
    def test_012_devices_list_invalid_ident(self):
        vm = TestVM({
            '/vanir-block-devices/invalid ident': b'',
            '/vanir-block-devices/invalid+ident': b'',
            '/vanir-block-devices/invalid#': b'',
        })
        devices = sorted(list(self.ext.on_device_list_block(vm, '')))
        self.assertEqual(len(devices), 0)
    ...
    def test_040_attach(self):
        ...
        vm = TestVM({}, domain_xml=domain_xml_template.format(''))
        dev = vanir.ext.block.BlockDevice(back_vm, 'sda')
    ...
    def test_046_attach_ro_dev_rw(self):
        back_vm = TestVM(name='sys-usb', qdb={
            ...
            '/vanir-block-devices/sda/desc': b'Test device',
            '/vanir-block-devices/sda/size': b'1024000',
            '/vanir-block-devices/sda/mode': b'r',
        })
        vm = TestVM({}, domain_xml=domain_xml_template.format(''))
        dev = vanir.ext.block.BlockDevice(back_vm, 'sda')
        with self.assertRaises(vanir.exc.VanirValueError):
