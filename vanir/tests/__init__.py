...
XMLPATH = '/var/lib/vanir/vanir-test.xml'
CLASS_XMLPATH = '/var/lib/vanir/vanir-class-test.xml'
TEMPLATE = 'fedora-23'
VMPREFIX = 'test-inst-'
...
if 'DEFAULT_LVM_POOL' in os.environ.keys():
    ...
    DEFAULT_LVM_POOL = 'vanir_dom0/pool00'
...
try:
    import libvirt
    libvirt.openReadOnly(vanir.config.defaults['libvirt_uri']).close()
    in_dom0 = True
...
try:
    ...
    vanir.log.LOGPATH = '/tmp'
    vanir.log.LOGFILE = '/tmp/vanir.log'
...
class _QrexecPolicyContext(object):
    ...
    def __init__(self, service, source, destination, allow=True, action=None):
        ...
        self._filename = pathlib.Path('/etc/vanir-rpc/policy') / service
        if action is None:
...
class VanirTestCase(unittest.TestCase):
    ...
    def __init__(self, *args, **kwargs):
        super(VanirTestCase, self).__init__(*args, **kwargs)
        self.longMessage = True
        self.log = logging.getLogger('{}.{}.{}'.format(
            self.__class__.__module__,
            self.__class__.__name__,
            self._testMethodName))
        self.addTypeEqualityFunc(vanir.devices.DeviceManager,
    ...
    def setUp(self):
        ...
        self.addCleanup(vanir.ext.pci._cache_get.cache_clear)
    ...
    def cleanup_traceback(self):
        ...
        for exc_info in exc_infos:
            ...
            while ex is not None:
                if isinstance(ex, vanir.exc.QubesVMError):
                    ex.vm = None
    ...
    def assertXMLIsValid(self, xml, file=None, schema=None):
        ...
        if schema is not None and file is None:
            ...
            if not os.path.isabs(file):
                basedirs = ['/usr/share/doc/vanir/relaxng']
                if in_git:
...
class SystemTestCase(VanirTestCase):
    """
    Mixin for integration tests. All the tests here should use self.app
    object and when need vanir.xml path - should use :py:data:`XMLPATH`
    ...
    def setUp(self):
        ...
        self.vanirsd = self.loop.run_until_complete(
            vanir.api.create_servers(
    ...
    def close_qdb_on_remove(self, app, event, vm, **kwargs):
        # only close VanirDB  connection, do not perform other (destructive)
        # actions of vm.close()
    ...
    def cleanup_app(self):
        ...
        self.loop.run_until_complete(asyncio.wait([
            server.wait_closed() for server in self.vanirsd]))
        del self.vanirsd

        # remove all references to any complex vanir objects, to release
        ...
        for attr in dir(self):
            ...
            if obj_type.__module__.startswith('vanir'):
                delattr(self, attr)
    ...
    def _find_pool(self, volume_group, thin_pool):
        ...
        pools = [p for p in self.app.pools
                 if issubclass(p.__class__, vanir.storage.lvm.ThinPool)]
        for pool in pools:
    ...
    def _remove_vm_vanir(self, vm):
        ...
        try:
            conn = libvirt.open(vanir.config.defaults['libvirt_uri'])
        except:  # pylint: disable=bare-except
            pass
        else:
            try:
    ...
    def _remove_vm_disk(vmname):
        for dirspec in (
                'vanir_appvms_dir',
                'vanir_servicevms_dir',
                'vanir_templates_dir'):
            dirpath = os.path.join(vanir.config.qubes_base_dir,
                vanir.config.system_path[dirspec], vmname)
    ...
    def remove_test_vms(self, xmlpath=XMLPATH, prefix=VMPREFIX):
        ...
        if os.path.exists(xmlpath):
            try:
                ...
                if not hasattr(self, 'app'):
                    app.close()
                del app
            except vanir.exc.VanirException:
                pass
            os.unlink(xmlpath)
        ...
        for dirspec in (
                ...
                'vanir_templates_dir'):
            dirpath = os.path.join(vanir.config.qubes_base_dir,
                vanir.config.system_path[dirspec])
    ...
    def wait_for_session(self, vm):
        ...
        yield from asyncio.wait_for(
            vm.run_service_for_stdio(
                'vanir.WaitForSession', input=vm.default_user.encode()),
            timeout=timeout)
...
def list_templates():
    ...
    if _templates is None:
        if 'VANIR_TEST_TEMPLATES' in os.environ:
...
def create_testcases_for_templates(name, *bases, module, **kwds):
    ...
    ...             'TC_00_MyTests', MyTestsMixIn, vanir.tests.SystemTestCase,
...
def maybe_create_testcases_on_import(create_testcases_gen):
    ...
    The conditions - any of:
     - VANIR_TEST_TEMPLATES present in the environment (it's possible to
     create test cases without opening vanir.xml)
     - VANIR_TEST_LOAD_ALL present in the environment
    '''
    if 'VANIR_TEST_TEMPLATES' in os.environ or \
            'VANIR_TEST_LOAD_ALL' in os.environ:
...
def extra_info(obj):
    ...
    if isinstance(obj, vanir.vm.vanirvm.QubesVM):
        try:
            return obj.name
        except AttributeError:
            pass
...
def load_tests(loader, tests, pattern): # pylint: disable=unused-argument
    ...
    for modname in (
            ...
            'vanir.tests.events',
            'vanir.tests.devices',
            'vanir.tests.devices_block',
            'vanir.tests.firewall',
            'vanir.tests.init',
            'vanir.tests.vm.init',
            'vanir.tests.storage',
            'vanir.tests.storage_file',
            'vanir.tests.storage_reflink',
            'vanir.tests.storage_lvm',
            'vanir.tests.storage_kernels',
            'vanir.tests.ext',
            'vanir.tests.vm.vanirvm',
            'vanir.tests.vm.mix.net',
            'vanir.tests.vm.adminvm',
            'vanir.tests.vm.appvm',
            'vanir.tests.vm.dispvm',
            'vanir.tests.app',
            'vanir.tests.tarwriter',
            'vanir.tests.api',
            'vanir.tests.api_admin',
            'vanir.tests.api_misc',
            'vanir.tests.api_internal',
            'vanirpolicy.tests',
            'vanirpolicy.tests.cli',
            ):
        tests.addTests(loader.loadTestsFromName(modname))

    for modname in (
            'vanirpolicy.tests.gtkhelpers',
            'vanirpolicy.tests.rpcconfirmation',
            ):
        tests.addTests(loader.loadTestsFromName(modname))
    ...
    for modname in (
            # integration tests
            'vanir.tests.integ.basic',
            'vanir.tests.integ.storage',
            'vanir.tests.integ.grub',
            'vanir.tests.integ.devices_block',
            'vanir.tests.integ.devices_pci',
            'vanir.tests.integ.dom0_update',
            'vanir.tests.integ.network',
            'vanir.tests.integ.dispvm',
            'vanir.tests.integ.vm_qrexec_gui',
            'vanir.tests.integ.mime',
            'vanir.tests.integ.salt',
            'vanir.tests.integ.backup',
            'vanir.tests.integ.backupcompatibility',
#           'vanir.tests.regressions',
