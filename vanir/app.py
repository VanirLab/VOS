...
class VMMConnection:
    ...
    def init_vmm_connection(self):
        ...
        self._libvirt_conn = VirConnectWrapper(
            vanir.config.defaults['libvirt_uri'])
...
class VanirHost:
    ...
    :param vanir.vanir app: vanir application context (must have \
        :py:attr:`vanir.vmm` attribute defined)
    '''
    ...
    def _fetch(self):
        ...
        self.app.log.debug('VanirHost: no_cpus={} memory_total={}'.format(
            self.no_cpus, self.memory_total))
        try:
            self.app.log.debug('VanirHost: xen_free_memory={}'.format(
...
class VMCollection:
    ...
    def add(self, value, _enable_events=True):
        ...
        :param vanir.vm.BaseVM value: VM to add
        ...
        if not isinstance(value, vanir.vm.BaseVM):
    ...
    def __getitem__(self, key):
        ...
        if isinstance(key, vanir.vm.BaseVM):
            key = key.uuid
    ...
    def __delitem__(self, key):
        vm = self[key]
        if not vm.is_halted():
            raise vanir.exc.VanirVMNotHaltedError(vm)
...
#               if vm.is_netvm():
    ...
    def get_new_unused_dispid(self):
        for _ in range(int(vanir.config.max_dispid ** 0.5)):
            dispid = random.SystemRandom().randrange(vanir.config.max_dispid)
...
def _default_pool(app):
    ...
    3. Look for file(-reflink)-based pool pointing to /var/lib/vanir
    4. Fail
    '''
...
def _setter_pool(app, prop, value):
    if isinstance(value, vanir.storage.Pool):
        return value
    try:
        return app.pools[value]
    except KeyError:
        raise vanir.exc.VanirPropertyValueError(app, prop, value,
            'No such storage pool')

def _setter_default_netvm(app, prop, value):
    # skip netvm loop check while loading vanir.xml, to avoid tricky loading
    # order
