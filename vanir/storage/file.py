...
class FilePool(vanir.storage.Pool):
    ...
    Rapid reference on device-mapper layers:
    ...
    def _vid_prefix(vm):
        ...
        import vanir.vm.templatevm  # pylint: disable=redefined-outer-name
    ...
    def target_dir(self, vm):
        """ Returns the path to vmdir depending on the type of the VM.
            The default VOS file storage saves the vm images in three
            different directories depending on the ``VanirVM`` type:
            * ``appvms`` for ``VanirAppVm`` or ``VanirHvm``
            * ``vm-templates`` for ``VanirTemplateVm`` or ``VanirTemplateHvm``
            Args:
                vm: a VanirVM
...
class FileVolume(vanir.storage.Volume):
    ...
    def block_device(self):
        ...
        if self.snap_on_start or self.save_on_stop:
            path += ":" + self.path_cow
        return vanir.storage.BlockDevice(path, self.name, self.script, self.rw,
