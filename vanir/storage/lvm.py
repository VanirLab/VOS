''' Driver for placing vm images in a LVM thin pool '''
import functools
import logging
import os
import subprocess
...
class ThinPool(vanir.storage.Pool):
    ...
    *volume_group*/*thin_pool* arguments. LVM volume naming scheme:
    ...
    def __init__(self, volume_group, thin_pool, revisions_to_keep=1, **kwargs):
        ...
        self.log = logging.getLogger('vanir.storage.lvm.%s' % self._pool_id)
    ...
    def init_volume(self, vm, volume_config):
        ''' Initialize a :py:class:`vanir.storage.Volume` from `volume_config`.
        '''
        ...
        if 'vid' not in volume_config.keys():
            if vm and hasattr(vm, 'name'):
                ...
                vm_name = vanir.utils.random_string()
    ...
    def size(self):
        try:
            return vanir.storage.lvm.size_cache[
...
def init_cache(log=logging.getLogger('vanir.storage.lvm')):
    cmd = _init_cache_cmd
    if os.getuid() != 0:
        cmd = ['sudo'] + cmd
...
@asyncio.coroutine
def init_cache_coro(log=logging.getLogger('vanir.storage.lvm')):
    cmd = _init_cache_cmd
    if os.getuid() != 0:
        cmd = ['sudo'] + cmd
    environ = os.environ.copy()
...
class ThinVolume(vanir.storage.Volume):
    ...
    def __init__(self, volume_group, size=0, **kwargs):
        ...
        self.log = logging.getLogger('vanir.storage.lvm.%s' % str(self.pool))
    ...
    def revisions(self):
        ...
        for revision_vid in size_cache:
            ...
            iso_date = vanir.storage.isodate(seconds).split('.', 1)[0]
    ...
    def size(self):
        try:
            if self.is_dirty():
                return vanir.storage.lvm.size_cache[self._vid_snap]['size']
            return vanir.storage.lvm.size_cache[self._vid_current]['size']
        except KeyError:
            return self._size
    ...
    def _reset(self):
        ...
        try:
            cmd = ['remove', self.vid]
            yield from vanir_lvm_coro(cmd, self.log)
    ...
    def revert(self, revision=None):
        ...
        if not os.path.exists(old_path):
            ...
            raise vanir.storage.StoragePoolException(msg)
    ...
    def resize(self, size):
        ''' Expands volume, throws
            :py:class:`qubst.storage.vanir.storage.StoragePoolException` if
    ...
    def block_device(self):
        ''' Return :py:class:`vanir.storage.BlockDevice` for serialization in
            the libvirt XML template as <disk>.
        '''
        if self.snap_on_start or self.save_on_stop:
            return vanir.storage.BlockDevice(
    ...
    def usage(self):  # lvm thin usage always returns at least the same usage as
                      # the parent
        try:
            return vanir.storage.lvm.size_cache[self._vid_current]['usage']
        except KeyError:
            return 0
...
def reset_cache():
    vanir.storage.lvm.size_cache = init_cache()

@asyncio.coroutine
def reset_cache_coro():
    vanir.storage.lvm.size_cache = yield from init_cache_coro()