...
class Volume:
    ''' Encapsulates all data about a volume for serialization to vanir.xml and
        libvirt config.
        Keep in mind!
    ...
    def resize(self, size):
        ''' Expands volume, throws
            :py:class:`vanir.storage.StoragePoolException` if
    ...
    @property
    def config(self):
        ''' return config data for serialization to vanir.xml '''
        result = {
            'name': self.name,
            'pool': str(self.pool),
            'vid': self.vid,
...
class Storage:
    ...
    def attach(self, volume, rw=False):
        ...
        # events itself
        # self.vm.untrusted_qdb.write('/vanir-block-devices', '')
        # ← do we need this?
    ...
    def resize(self, volume, size):
        ...
        if self.vm.is_running():
            try:
                yield from self.vm.run_service_for_stdio('vanir.ResizeDisk',
    ...
    def clone_volume(self, src_vm, name):
        ...
        :param VanirVM src_vm: source VM
...
class Pool:
    ''' A Pool is used to manage different kind of volumes (File
        ...
        this class.
    '''  # pylint: disable=unused-argument
    private_img_size = vanir.config.defaults['private_img_size']
    root_img_size = vanir.config.defaults['root_img_size']
    ...
    @property
    def config(self):
        ''' Returns the pool config to be written to vanir.xml '''
    ...
    def included_in(self, app):
        ...
        :param app: vanir() object to lookup other pools in
...
def driver_parameters(name):
    ...
    init_function = vanir.utils.get_entry_point_one(
        vanir.storage.STORAGE_ENTRY_POINT, name).__init__
