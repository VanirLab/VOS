...
#: constant returned when some action should be performed on all vanir
VM_ALL = object()


class VanirAction(argparse.Action):
...
class HelpPropertiesAction(argparse.Action):
    ...
    def __init__(self,
        ...
        import vanir # pylint: disable=redefined-outer-name
        self._klass = klass if klass is not None else vanir.vanir
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        text = 'Common properties:\n' + '\n'.join(
            wrapper.fill('{name:{width}s}  {doc}'.format(
                name=prop.__name__,
                doc=vanir.utils.format_doc(prop.__doc__) if prop.__doc__ else'',
                width=width))
            for prop in sorted(properties))
        if self._klass is not vanir.vanir:
            text += '\n\n' \
                'There may be more properties in specific domain classes.\n'
        parser.exit(message=text)


class VmNameAction(VanirAction):
...
class VolumeAction(VanirAction):
    ''' Action for argument parser that gets the
        :py:class:``vanir.storage.Volume`` from a POOL_NAME:VOLUME_ID string.
    '''
    # pylint: disable=too-few-public-methods
...
class VanirArgumentParser(argparse.ArgumentParser):
    '''Parser preconfigured for use in most of the vanir command-line tools.
    :param bool want_app: instantiate :py:class:`vanir.vanir` object
    :param bool want_app_no_instance: don't actually instantiate \
        :py:class:`vanir.vanir` object, just add argument for custom xml file
    ...
    *kwargs* are passed to :py:class:`argparser.ArgumentParser`.
    ...
    def __init__(self, want_app=True, want_app_no_instance=False,
        ...
        super(VanirArgumentParser, self).__init__(**kwargs)
...
class VmNameGroup(argparse._MutuallyExclusiveGroup):
    ...
    def __init__(self, container, required, vm_action=VmNameAction, help=None):
        ...
        if not help:
            help = 'perform the action on all vanir'
